...
| **F-ME4** — Supported-file visibility | Render only explicitly supported formats; report unsupported files without putting them on the canvas | PR-010 | P1 |
...
| **PR-010** — Supported-file visibility | The canvas renders only folders and explicitly supported files: PNG, JPEG, GIF, SVG, WebP, MP4, MOV, PDF, and Markdown. Other formats, including HTML and rich documents, remain untouched and are hidden by default. The interface reports the number of unsupported files hidden in the current directory. Unsupported content is never interpreted or rendered inside terrazzo. A future reveal control may expose generic system-open cards, but it is not part of v0. Rust still detects unsupported files so hidden is distinct from missing: new unsupported files receive no layout entry, and an existing layout entry is retained without becoming a tombstone while its file is unsupported. | F-ME4 | P1 |
...
| **PR-014** — Async thumbnails | Poster frames and supported-file thumbnails generate in a background process; the canvas shows a placeholder until they land. UI never blocks on import. | F-ST2 | P0 |
| **PR-015** — File watching | A filesystem watcher reconciles the canvas live: new supported files get a default placement, deleted supported files get a tombstone (not a crash), modified supported files refresh their preview, and unsupported-file changes update the hidden count without creating cards. | F-ST3 | P1 |
...
| **PR-022** — Graceful degradation | Missing supported files, aliases, permission failures, and symlinks resolving outside the workspace root render as labeled tombstones—never a crash, never silent omission. Unsupported files are intentionally hidden under PR-010 and do not become tombstones. | P1 |
